...
```
```{r}
p1.h1 <- hypothesis(p1.brms.model, "audiosound = -.3")
p1.h1
plot(p1.h1)
```
...
p1.single.trial.brms.model.priors <- c(
...
p1.single.trial.brms.model <- brm(
  ...
  chains = 6,
  cores = 6,
  file = "data/brms_fits/p1_single_trial_model"
)
...
p1.congruence.brms.model.formula <- bf(
  is.congruent ~ 
    ...
    peak.time:audio +
    peak.amplitude:audio +
    peak.time*peak.amplitude | subject) + (peak.amplitude:peak.time:audio +
    peak.time:audio +
    peak.amplitude:audio +
...
p1.congruence.brms.model.priors <- c(
...
p1.congruence.brms.model <- brm(
  ...
  chains = 6,
  cores = 6,
  file = "data/brms_fits/p1_congruence_single_trial_model"
)
...
**BRMS version**
```{r}
n4.single.trial.brms.model.formula <- bf(rt ~ mean.amplitude.z + audio + congruence  + (mean.amplitude.z + audio + congruence | subject) + (mean.amplitude.z + audio + congruence | stimulus))

get_prior(n4.single.trial.brms.model.formula, data=n4.single.trial.model.data)

n4.single.trial.brms.model.priors <- c(
  set_prior("normal(0,100)", class="b")
)

n4.single.trial.brms.model <- brm(
  formula = n4.single.trial.brms.model.formula,
  data = n4.single.trial.model.data,
  prior = n4.single.trial.brms.model.priors,
  warmup = 1500,
  iter = 3000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 6,
  cores = 6,
  file = "data/brms_fits/n4_single_trial_model"
)
