...
At the request of the reviewers, we also fit an exploratory model with a maximal random effects structure, including random effects for stimulus. For these models, we use Bayesian estimation. 
...
Here we try a different analysis strategy at the suggestion of a reviewer. We fit a maximal random-effects structure for each of the ERP component models using Bayesian estimation. We set the priors of the population-level parameters based on the reported effects in B&L. For each effect, we set a prior of a normal distribution centered on 0 with standard deviation equal to the reported effect in the paper. This is a conservative way to estimate the degree to which our data strengthens beliefs in the null hypothesis, because the null has the highest prior probability in our model, and the size of the reported effect in B&L controls how dispersed the prior probability is. Stronger effects have more dispersed prior probability, and so are less concentrated at the null of 0. We then use bayes factors to quantify the degree to which our data shifts beliefs in the null hypothesis.
...
```{r}
p2.h1 <- hypothesis(p2.brms.model, "audiosound = 0")
p2.h1
plot(p2.h1)
```
```{r}
p2.h2 <- hypothesis(p2.brms.model, "congruencenonMmatch > 0")
p2.h2
plot(p2.h2)
```

```{r}
p2.h3 <- hypothesis(p2.brms.model, "audiosound:congruencenonMmatch = 0")
p2.h3
plot(p2.h3)
```
...
```{r}
n4.h1 <- hypothesis(n4.brms.model, "congruencenonMmatch = 0")
plot(n4.h1)
```

```{r}
n4.h1.2 <- hypothesis(n4.brms.model, "congruencenonMmatch = -0.82")
n4.h1.2
```

```{r}
n4.h2 <- hypothesis(n4.brms.model, "audiosound = 0")
plot(n4.h2)
n4.h2
```

```{r}
n4.h3 <- hypothesis(n4.brms.model, "audiosound:congruencenonMmatch = 0")
plot(n4.h3)
n4.h3
```
...
summary(p1.single.trial.brms.model)
```

```{r}
p1.single.trial.h1 <- hypothesis(p1.single.trial.brms.model, "peak.time = 0")
p1.single.trial.h1
plot(p1.single.trial.h1)
```

```{r}
p1.single.trial.h2 <- hypothesis(p1.single.trial.brms.model, "peak.amplitude = 0")
p1.single.trial.h2
plot(p1.single.trial.h2)
```
...
```{r}
p1.congruence.h1 <- hypothesis(p1.congruence.brms.model, "peak.time:audiosound = 0")

p1.congruence.h1
plot(p1.congruence.h1)
```

```{r}
mcmc_plot(p1.congruence.brms.model, type="trace")
```
