...
beh.model <- lmer(rt ~ match_type*audio_type + (audio_type | participant_id) + (match_type | participant_id), data=beh.model.data, control=lmerControl(optimizer="Nelder_Mead"))
summary(beh.model)
```
...
eeg.p1.result <- lmer(mean.amplitude ~ hemisphere*audio*congruence + (hemisphere|subject) + (congruence|subject) + (audio|subject), data=eeg.p1.model.data, control=lmerControl(optimizer="optimx", optCtrl = list(method="Nelder-Mead")))
...
p1.peak.latency.model <- lmer(rt ~ peak.time + peak.amplitude + audio_type + match_type + (audio_type|subject) + (match_type|subject), data=p1.peak.latency.data, control=lmerControl(optimizer = "bobyqa"))

summary(p1.peak.latency.model)

plot(p1.peak.latency.data$peak.time, p1.peak.latency.data$rt)
```