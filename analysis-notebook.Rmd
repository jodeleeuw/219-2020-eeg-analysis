...
p1.single.trial.model <- lmer(rt ~ peak.time + peak.amplitude + audio + congruence + (audio + congruence | subject) + (audio + congruence | image_category), data=p1.single.trial.model.data, control=lmerControl(optimizer="nloptwrap"))

p1.single.trial.model.simpler.random.effects <- lmer(rt ~ peak.time + peak.amplitude + audio + congruence + (1 + audio + congruence | subject) + (1 | image_category), data=p1.single.trial.model.data)
```
...
p1.congruence.model.v5 <- glmer(
  is.congruent ~
    ...
    peak.amplitude.z:audio +
    peak.time.z:audio +
    peak.amplitude.z:peak.time.z:audio | subject), p1.congruence.model.data, family="binomial")
...
```{r}
summary(p1.congruence.brms.model.MI)
```
...
```{r}
summary(p1.congruence.brms.model.BL)
```

```{r}
hypothesis (p1.congruence.brms.model.BL, "peak.amplitude=0") %>% plot()
```
