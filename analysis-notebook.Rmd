...
```{r}
rt.full.dist.data <- behavioral.data %>%
  filter(phase=="test", correct==TRUE) %>%
  group_by(participant_id, match_type, audio_type) %>%
  summarize(participant.avg.rt = mean(rt)) %>%
  pivot_wider
...
```{r}
behavioral.figure.full <- ggplot(rt.full.dist.data, aes(x=match_type, y=participant.avg.rt, color=audio_type))+
  geom_violin()+
  labs(x="Congruence", y="Response Time (ms)", fill="Cue Type")+
  scale_fill_brewer(type="qual", palette = "Set1", labels=c("Label", "Sound"))+
  scale_x_discrete(labels=c("Match", "Mismatch"))+
...
rt.model <- lmer.cached("data/lmer_fits/rt_model",
                        rt ~ audio_type*match_type + (audio_type + match_type|participant_id), 
                        data=rt.model.data,
...
rt.brms.model.priors.BL <- c(
  set_prior("normal(10.4,1.93)", class="b", coef="audio_typesound"),
  set_prior("normal(30,1.94)", class="b", coef="match_typenonMmatch"),
  set_prior("normal(0,10.4)", class="b", coef="audio_typesound:match_typenonMmatch")
)

rt.brms.model.priors.MI <- c(
  set_prior("normal(0, 60)", class="b")
)

rt.brms.model.BL <- brm(
  formula = rt.brms.model.formula,
  data = rt.brms.model.data,
  prior = rt.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  ...
  chains = 8,
  cores = 8,
  file = "data/brms_fits/rt_model_BL"
)

rt.brms.model.MI <- brm(
  formula = rt.brms.model.formula,
  data = rt.brms.model.data,
  prior = rt.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/rt_model_MI"
)
```

#### Analysis of MI model

With the brms maximal random effects model we can look at the parameter estimates on the population-level parameters:

```{r}
summary(rt.brms.model.MI)
```


#### Analysis of BL model

```{r}
summary(rt.brms.model.BL)
```

Has our belief in the alternative hypothesis been strengthened?

```{r}
rt.h1 <- hypothesis(rt.brms.model.BL, "audio_typesound = 0")
rt.h1
```
...
p1.model <- lmer.cached(
  "data/lmer_fits/p1_amplitude_model",
  mean.amplitude ~ hemisphere*audio*congruence + (hemisphere + audio + congruence|subject), data=p1.model.data, control=lmerControl(optimizer="bobyqa"))
...
p2.model <- lmer.cached(
  "data/lmer_fits/p2_amplitude_model",
  peak.amplitude ~ hemisphere*audio*congruence + (hemisphere + audio + congruence|subject), 
  data=p2.model.data
)
...
```
```{r}
p2.model.simpler <- lmer.cached(
  "data/lmer_fits/p2_amplitude_model_simplified",
  peak.amplitude ~ hemisphere*audio*congruence + (audio + congruence|subject), 
  data=p2.model.data
)

summary(p2.model.simpler)
```
...
p1.p2.interaction.plot.a <- ggplot(p1.p2.follow.up.interaction.data, aes(x=t, y=M, ymin=M-SE, ymax=M+SE, color=audio, fill=audio))+
  geom_vline(xintercept = 0, color="grey20")+
  geom_hline(yintercept = 0, color="grey20")+
  geom_ribbon(alpha=0.2, color=NA)+
  geom_line(size=0.75)+
...
p1.p2.interaction.plot.b <- ggplot(p1.p2.follow.up.interaction.data, aes(x=t, y=M, ymin=M-SE, ymax=M+SE, color=congruence, fill=congruence))+
  geom_vline(xintercept = 0, color="grey20")+
  geom_hline(yintercept = 0, color="grey20")+
  geom_ribbon(alpha=0.2, color=NA)+
  geom_line(size=0.75)+
  facet_grid(audio~.)+
  facet_grid(audio~., labeller = labeller(audio=c(label="Label", sound="Sound")))+
  scale_x_continuous(expand=c(0,0))+
  scale_y_continuous(breaks=c(-2,0,2,4,6))+
  labs(title=NULL, x="Time since target onset (ms)", y="Amplitude (\U003BCV)", fill="Congruence", color="Congruence")+
  scale_color_brewer(type="qual", palette = "Set2", labels=c("Match", "Mismatch") )+
  scale_fill_brewer(type="qual", palette = "Set2", labels=c("Match", "Mismatch"))+
  theme_bw(base_size = 9) +
  coord_cartesian(xlim=c(-100,300),ylim=c(-2.5,6.5))+
...
merged.fig.5 <- p1.p2.interaction.plot.a | p1.p2.interaction.plot.b

ggsave("figures/figure5-p1-p2-interaction.png", merged.fig.5, height = 3.5, width=7, units="in", dpi=300)
...
get_prior(p1.brms.model.formula, data=p1.model.data)

p1.brms.model.priors.BL <- c(
  set_prior("normal(0, 0.3)", class="b"),
  set_prior("normal(-0.3, 0.127)", coef="audiosound"),
  set_prior("normal(-0.2, 0.22)", coef="hemisphereright:audiosound")
)

p1.brms.model.priors.MI <- c(
  set_prior("normal(0, 0.6)", class="b")
)

p1.brms.model.BL <- brm(
  formula = p1.brms.model.formula,
  data = p1.model.data,
  prior = p1.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_model_BL"
)

p1.brms.model.MI <- brm(
  formula = p1.brms.model.formula,
  data = p1.model.data,
  prior = p1.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_model_MI"
)
```

### P1 analysis

#### MI priors parameter estimates

Summarize the P1 model fits
```{r}
summary(p1.brms.model.MI)
```

#### Analysis of BFs using the BL priors

```{r}
summary(p1.brms.model.BL)
```

```{r}
p1.h1 <- hypothesis(p1.brms.model.BL, "audiosound = 0")
p1.h1
```


```{r}
p1.h2 <- hypothesis(p1.brms.model.BL, "hemisphereright:audiosound = 0")
p1.h2
```
...
get_prior(p2.brms.model.formula, data=p2.model.data)


p2.brms.model.priors.BL <- c(
  set_prior("normal(0,0.31)", class="b"),
  set_prior("normal(-0.31,0.11)", coef="audiosound"),
  set_prior("normal(0.5,0.11)", coef="congruencenonMmatch")
)

p2.brms.model.priors.MI <- c(
  set_prior("normal(0,1)", class="b")
)

p2.brms.model.BL <- brm(
  formula = p2.brms.model.formula,
  data = p2.model.data,
  prior = p2.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p2_model_BL"
)

p2.brms.model.MI <- brm(
  formula = p2.brms.model.formula,
  data = p2.model.data,
  prior = p2.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p2_model_MI"
)
```


```{r}
summary(p2.brms.model.MI)
```

```{r}
summary(p2.brms.model.BL)
```


```{r}
p2.h1 <- hypothesis(p2.brms.model.BL, "audiosound = 0")
p2.h1
```
...
```
```{r}
p2.h2 <- hypothesis(p2.brms.model.BL, "congruencenonMmatch = 0")
p2.h2
```
```{r}
plot(p2.h2)
...
n4.model <- lmer.cached(
  "data/lmer_fits/n4_amplitude_model",
  mean.amplitude ~ audio*congruence + (audio + congruence | subject), 
  data=n4.model.data,
...
get_prior(n4.brms.model.formula, data=n4.model.data)

n4.brms.model.priors.BL <- c(
  set_prior("normal(0,0.82)", class="b"),
  set_prior("normal(-0.1,0.09)", coef="audiosound"),
  set_prior("normal(-0.82,0.13)", coef="congruencenonMmatch")
)

n4.brms.model.priors.MI <- c(
  set_prior("normal(0,1.64)", class="b")
)

n4.brms.model.BL <- brm(
  formula = n4.brms.model.formula,
  data = n4.model.data,
  prior = n4.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/n4_model_BL"
)

n4.brms.model.MI <- brm(
  formula = n4.brms.model.formula,
  data = n4.model.data,
  prior = n4.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/n4_model_MI"
)
```


```{r}
summary(n4.brms.model.MI)
```

```{r}
summary(n4.brms.model.BL)
```

```{r}
n4.h1 <- hypothesis(n4.brms.model.BL, "congruencenonMmatch = 0")
n4.h1
```
```{r}
plot(n4.h1)
...
```{r}
n4.h2 <- hypothesis(n4.brms.model.BL, "audiosound = 0")
n4.h2
```


## Merged Waveform Visulization Figure
...
### P1: Do amplitude and latency predict response times?
...
p1.single.trial.model.data <- eeg.single.trial.data %>%
  filter(component=="P1", !is.na(peak.time), hemisphere == "left")
  # we can pick hemisphere left/right arbitrarily, because this measurement is averaged
  # over hemisphere in the pre-processing step. this just removes the duplicate value
  # since the left/right versions are the same.

p1.single.trial.model <- lmer.cached(
  "data/lmer_fits/p1_single_trial_model",
  rt ~ peak.time + peak.amplitude + audio + congruence + (audio + congruence | subject) + (audio + congruence | image_category),
  data=p1.single.trial.model.data)

p1.single.trial.model.simpler.random.effects <- lmer.cached(
  "data/lmer_fits/p1_single_trial_model_simpler",
  rt ~ peak.time + peak.amplitude + audio + congruence + (1 + audio + congruence | subject) + (1 | image_category), 
  data=p1.single.trial.model.data,
  control=lmerControl(optimizer="bobyqa"))
```
...
```{r}
summary(p1.single.trial.model.simpler.random.effects)
```
...
p1.single.trial.brms.model.formula <- bf(rt ~ peak.time + peak.amplitude + audio + congruence + (peak.time + peak.amplitude + audio + congruence | subject) + (peak.time + peak.amplitude + audio + congruence | stimulus))

get_prior(p1.single.trial.brms.model.formula, data=p1.single.trial.model.data)

p1.single.trial.brms.model.priors.BL <- c(
  set_prior("normal(0.23,0.10)", coef="peak.time"),
  set_prior("normal(-0.56,0.22)", coef="peak.amplitude"),
  set_prior("normal(10.4,1.93)", coef="audiosound"),
  set_prior("normal(30,1.94)", coef="congruencenonMmatch")
)

p1.single.trial.brms.model.priors.MI <- c(
  set_prior("normal(0,1.12)", coef="peak.time"),
  set_prior("normal(0,1.12)", coef="peak.amplitude"),
  set_prior("normal(0,60)", coef="audiosound"),
  set_prior("normal(0,60)", coef="congruencenonMmatch")
)

p1.single.trial.brms.model.BL <- brm(
  formula = p1.single.trial.brms.model.formula,
  data = p1.single.trial.model.data,
  prior = p1.single.trial.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_single_trial_model_BL"
)

p1.single.trial.brms.model.MI <- brm(
  formula = p1.single.trial.brms.model.formula,
  data = p1.single.trial.model.data,
  prior = p1.single.trial.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_single_trial_model_MI"
)
```

```{r}
summary(p1.single.trial.brms.model.MI)
```


```{r}
summary(p1.single.trial.brms.model.BL)
```

```{r}
hypothesis(p1.single.trial.brms.model.BL, "peak.time=0")
```

```{r}
hypothesis(p1.single.trial.brms.model.BL, "peak.amplitude = 0")
```

### P1: Is congruence predictable from amplitude, latency, and cue type?
...
```{r}
p1.congruence.model.v1 <- glmer.cached(
  "data/lmer_fits/p1_congruence_model_v1",
  is.congruent ~ peak.amplitude*peak.time*audio + (audio | subject) + (audio | image_category), 
  p1.congruence.model.data, 
  family="binomial", 
...
```{r}
p1.congruence.model.v2 <- glmer.cached(
  "data/lmer_fits/p1_congruence_model_v2",
...
```{r}
p1.congruence.model.v3 <- glmer.cached(
  "data/lmer_fits/p1_congruence_model_v3",
...
```{r}
p1.congruence.model.v4 <- glmer.cached(
  "data/lmer_fits/p1_congruence_model_v4",
...
**Model 5:** removing any main effects of audio, since the design of the experiment is such that
audio type should not predict congruence.

```{r}
p1.congruence.model.v5 <- glmer.cached(
  "data/lmer_fits/p1_congruence_model_v5",
  is.congruent ~
    peak.amplitude.z +
    peak.time.z +
    peak.amplitude.z:peak.time.z +
    peak.amplitude.z:audio +
    peak.time.z:audio +
    peak.amplitude.z:peak.time.z:audio +
    (0 + peak.amplitude.z +
    peak.time.z +
    peak.amplitude.z:peak.time.z +
    peak.amplitude.z:audio +
    peak.time.z:audio +
    peak.amplitude.z:peak.time.z:audio | subject), p1.congruence.model.data, family="binomial", control=glmerControl(optimizer = "bobyqa"))

summary(p1.congruence.model.v5)
```
...
#### Exploratory BRMS version

```{r}
p1.congruence.brms.model.formula <- bf(
  is.congruent ~ 
    peak.amplitude:peak.time:audio +
    peak.time:audio +
    peak.amplitude:audio +
    peak.time*peak.amplitude +
    (peak.amplitude:peak.time:audio +
    peak.time:audio +
    peak.amplitude:audio +
    peak.time*peak.amplitude | subject) + 
    (peak.amplitude:peak.time:audio +
    peak.time:audio +
    peak.amplitude:audio +
    peak.time*peak.amplitude | stimulus), family=bernoulli(link = "logit"))

get_prior(p1.congruence.brms.model.formula, data=p1.congruence.model.data)

p1.congruence.brms.model.priors.BL <- c(
  set_prior("normal(0, 0.42)", class="b"),
  set_prior("normal(0.42, 0.11)", coef="peak.time:audiosound"),
  set_prior("normal(-0.56, 0.25)", coef="peak.amplitude:audiosound")
)

p1.congruence.brms.model.priors.MI <- c(
  set_prior("normal(0, 1.12)", class="b")
)

p1.congruence.brms.model.BL <- brm(
  formula = p1.congruence.brms.model.formula,
  data = p1.congruence.model.data,
  prior = p1.congruence.brms.model.priors.BL,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_congruence_single_trial_model_BL"
)

p1.congruence.brms.model.MI <- brm(
  formula = p1.congruence.brms.model.formula,
  data = p1.congruence.model.data,
  prior = p1.congruence.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_congruence_single_trial_model_MI"
)
```

```{r}
print(summary(p1.congruence.brms.model.MI), digits=4)
```

```{r}
mcmc_plot(p1.congruence.brms.model.MI, pars=c("peak.time", "peak.amplitude"))
```
```{r}
pp_check(p1.congruence.brms.model.MI,x="peak.time")
```

```{r}
print(summary(p1.congruence.brms.model.BL), digits=4)
```

```{r}
hypothesis (p1.congruence.brms.model.BL, "peak.time:audiosound=0")
```

```{r}
hypothesis (p1.congruence.brms.model.BL, "peak.amplitude:audiosound=0")
```
...
```{r}
n4.single.trial.model <- lmer.cached(
  "data/lmer_fits/n4_single_trial_model_v1",
  rt ~ mean.amplitude.z + audio + congruence  + (mean.amplitude.z + audio + congruence | subject), 
  data=n4.single.trial.model.data, control=lmerControl(optimizer = "bobyqa"))
...
```{r}
n4.single.trial.model.v2 <- lmer.cached(
  "data/lmer_fits/n4_single_trial_model_v2",
  rt ~ mean.amplitude + audio + congruence + (mean.amplitude + audio + congruence | subject), data=n4.single.trial.model.data, control=lmerControl(optimizer = "bobyqa"))

summary(n4.single.trial.model.v2)
```

**Model 3** Variant without cue type and congruence as effects.

```{r}
n4.single.trial.model.v3 <- lmer.cached(
  "data/lmer_fits/n4_single_trial_model_v3",
  rt ~ mean.amplitude.z + (mean.amplitude.z | subject), 
  data=n4.single.trial.model.data, control=lmerControl(optimizer = "bobyqa"))

summary(n4.single.trial.model.v3)
```
...
n4.single.trial.model.v4 <- lmer.cached(
  "data/lmer_fits/n4_single_trial_model_v4",
  rt ~ mean.amplitude.z + audio + congruence + (mean.amplitude.z + audio + congruence | subject), data=n4.single.trial.model.data.filtered, control=lmerControl(optimizer = "bobyqa"))

summary(n4.single.trial.model.v4)
```

**BRMS version**
```{r}
n4.single.trial.brms.model.formula <- bf(rt ~ mean.amplitude + audio + congruence  + (mean.amplitude + audio + congruence | subject) + (mean.amplitude + audio + congruence | stimulus))

get_prior(n4.single.trial.brms.model.formula, data=n4.single.trial.model.data)

n4.single.trial.brms.model.priors.MI <- c(
  set_prior("normal(0,60)", coef="audiosound"),
  set_prior("normal(0,60)", coef="congruencenonMmatch"),
  set_prior("normal(0,10)", coef="mean.amplitude")
)

n4.single.trial.brms.model.MI <- brm(
  formula = n4.single.trial.brms.model.formula,
  data = n4.single.trial.model.data,
  prior = n4.single.trial.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/n4_single_trial_model_MI"
)
```

```{r}
summary(n4.single.trial.brms.model.MI)
```

## Exploratory, predicting RT with both P1 and N4 amplitude

```{r}
p1.part <- eeg.single.trial.data %>%
  filter(component=="P1", hemisphere=="left") %>%
  select(subject, trial, rt, congruence, audio, stimulus, peak.amplitude, peak.time) %>%
  mutate(p1.peak.amplitude = peak.amplitude, p1.peak.time = peak.time) %>%
  select(-peak.amplitude, -peak.time)

n4.part <- eeg.single.trial.data %>%
  filter(component=="N4") %>% 
  select(subject, trial, congruence, audio, mean.amplitude) %>% 
  mutate(n4.amplitude = mean.amplitude) %>% 
  select(-mean.amplitude)

p1.vs.n4.single.trial.model.data <- p1.part %>%
  left_join(n4.part, by=c("subject", "trial", "congruence", "audio")) %>%
  mutate(p1.peak.amplitude.z = scale(p1.peak.amplitude),
         p1.peak.time.z = scale(p1.peak.time),
         n4.amplitude.z = scale(n4.amplitude))
```

```{r}
p1.vs.n4.single.trial.brms.model.formula <- bf(rt ~ 
  p1.peak.amplitude.z + p1.peak.time.z + n4.amplitude.z + audio + congruence  + 
  (p1.peak.amplitude.z + p1.peak.time.z + n4.amplitude.z + audio + congruence | subject) + 
  (p1.peak.amplitude.z + p1.peak.time.z + n4.amplitude.z + audio + congruence | stimulus))

p1.vs.n4.single.trial.brms.model.priors.MI <- c(
  set_prior("normal(0,100)", class="b"),
  set_prior("normal(0,60)", coef="audiosound"),
  set_prior("normal(0,60)", coef="congruencenonMmatch")
)

p1.n4.single.trial.brms.model.MI <- brm(
  formula = p1.vs.n4.single.trial.brms.model.formula,
  data = p1.vs.n4.single.trial.model.data,
  prior = p1.vs.n4.single.trial.brms.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_vs_n4_single_trial_model_MI"
)
```

```{r}
summary(p1.n4.single.trial.brms.model.MI)
```


```{r}
p1.exploratory.latency.model.data <- eeg.single.trial.data %>%
  filter(component == "P1") %>% filter(hemisphere == 'left')

p1.exploratory.latency.model.formula <- bf(peak.time ~ audio * congruence + (audio * congruence | subject) + (audio * congruence | stimulus))

p1.exploratory.latency.model.priors.MI <- c(
  set_prior("normal(0,20)", class="b")
)

p1.exploratory.latency.brms.model.MI <- brm(
  formula = p1.exploratory.latency.model.formula,
  data = p1.exploratory.latency.model.data,
  prior = p1.exploratory.latency.model.priors.MI,
  warmup = 1000,
  iter = 4000,
  save_pars = save_pars(group=FALSE),
  sample_prior = TRUE,
  chains = 8,
  cores = 8,
  file = "data/brms_fits/p1_latency_model_MI"
)
```

```{r}
summary(p1.exploratory.latency.brms.model.MI)
```
